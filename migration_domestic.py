...
# =================================================================
# 3. 任务二：批量转存演员头像 (actors)
# =================================================================
def migrate_actor_avatars():
    print("\n==================================================")
    print("👤 开始处理 [演员头像] 转存任务...")
    print("==================================================")
    try {
        # 每次获取 200 位演员进行处理
        resp = requests.get(f"{base_api}/api/actors-migration-pending?limit=200", headers=headers, timeout=15)
        if resp.status_code != 200:
            print(f"❌ 获取待处理头像名单失败: {resp.text}")
            return
        
        actors = resp.json().get("data", [])
        if not actors:
            print("🎉 赞！所有国产区演员的头像都已完成转存。")
            return
            
        print(f"📦 发现 {len(actors)} 位演员头像等待转存...")
        
        for idx, actor in enumerate(actors):
            name = actor["name"]
            orig_url = actor["avatar_url"]
            
            # 相对路径智能保底补全
            if not orig_url.startswith(('http://', 'https://')):
                download_url = BACKUP_DOWNLOAD_HOST.rstrip('/') + "/" + orig_url.lstrip('/')
                print(f"[{idx+1}/{len(actors)}] 处理演员头像(补全): {name} -> {download_url}")
            else:
                download_url = orig_url
                print(f"[{idx+1}/{len(actors)}] 处理演员头像: {name} -> {download_url}")
            ...
            try {
                ...
                if upload_resp.status_code == 200:
                    ...
                    if isinstance(result, list) and len(result) > 0:
                        ...
                        # 3.3 成功后回写到 Cloudflare D1
                        update_payload = {
                            "name": name,
                            "avatar_url": new_img_url
                        }
                        update_resp = requests.post(f"{base_api}/api/actor-migration-update", json=update_payload, headers=headers, timeout=15)
                        if update_resp.status_code == 200:
                            print(f"  ✅ 数据库已同步更新！")
                        else:
                            print(f"  ❌ 回写数据库失败: {update_resp.text}")
        ...
        print(f"❌ 连接 Cloudflare 演员接口失败: {e}")

if __name__ == "__main__":
    migrate_movie_covers()
    migrate_actor_avatars()
    print("\n🎉 本轮国产区资产转存任务全部执行完毕！")
